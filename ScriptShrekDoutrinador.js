async function enviarScript(scriptText){
	...
	for(const line of lines){
		...
		// O WhatsApp moderno exige o evento 'input' para ativar o botão de enviar
		textarea.dispatchEvent(new Event('input', {bubbles: true}));
	
		// Pequena pausa para o botão de enviar aparecer no DOM
		await new Promise(resolve => setTimeout(resolve, 100));

		const botaoEnviar = main.querySelector(`[data-testid="send"]`) || 
						   main.querySelector(`[data-icon="send"]`)?.parentElement ||
						   main.querySelector(`span[data-icon="send"]`)?.closest('button');

		if (botaoEnviar) {
			botaoEnviar.click();
		} else {
			// Se o botão falhar, tenta o Enter físico
		...
		// Delay entre as mensagens (ajustado para 600ms para evitar bloqueios)
		if(lines.indexOf(line) !== lines.length - 1) await new Promise(resolve => setTimeout(resolve, 600));
	}
	
	return lines.length;
}

// Cole o roteiro do Shrek aqui dentro das crases:
enviarScript(`
Once upon a time there was a lovely princess.
But she had an enchantment upon her of a fearful sort 
which could only be broken by love's first kiss.
She was locked in a castle guarded by a terrible fire-breathing dragon.
